...
- [ ] T033 [US1] UploadWidget island in `src/components/UploadWidget/` + `.module.css`: native file input, two consent checkboxes, the UploadSubmission state machine, aria-live status messages ("Reading clauses", "Checking interest rates"); lazy-loadable — depends on T020, T031, T032 (FR-038, FR-039)
...
- [ ] T064 Comprehension & skeptic validation: usability script in `docs/` verifying one-scroll understanding (what/why/who/funding/contract-handling/what-you-get) and honest answers to catch/data/after-report (SC-001, SC-002)
